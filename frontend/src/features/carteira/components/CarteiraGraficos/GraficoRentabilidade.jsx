  ...
  return (
    <ResponsiveContainer width="100%" height={280}>
      <BarChart data={dados} layout="vertical" margin={{ top: 8, right: 16, left: 8, bottom: 0 }}>
        ...
        />
        <Tooltip
          content={({ active, payload, label }) => <ChartTooltip active={active} payload={payload} label={label} tipo="percentual" />}
        />
        <Bar dataKey="percentual" name="Rentabilidade" radius={[0, 4, 4, 0]}>
          {dados.map((item) => (
            <Cell key={item.ticker} fill={item.positivo ? COR_POSITIVA : COR_NEGATIVA} />
          ))}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
