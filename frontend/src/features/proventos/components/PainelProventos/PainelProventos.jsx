  ...
  const colunasProventos = [
    {
      ...
      render: (provento) => formatarData(provento.dataPagamento),
    },
    {
      chave: 'ticker',
      titulo: 'Ticker',
      classeCelula: 'font-medium text-white',
      render: (provento) => (
        <>
          {provento.ticker}
          {!provento.ativoId && (
    ...
    },
    {
      ...
      titulo: 'Tipo',
      classeCelula: 'text-slate-300',
      render: (provento) => TIPOS_PROVENTO[provento.tipo] ?? 'Provento',
    },
    {
      chave: 'quantidade',
      titulo: 'Qtd',
      classeCelula: 'tabular-nums text-slate-300',
      render: (provento) => numero.format(provento.quantidade),
    },
    {
      chave: 'valorPorCota',
      titulo: 'Valor/cota',
      classeCelula: 'tabular-nums text-slate-300',
      render: (provento) => moeda.format(provento.valorPorCota),
    },
    {
      chave: 'valorTotal',
      titulo: 'Total',
      classeCelula: 'tabular-nums font-medium text-emerald-300',
      render: (provento) => moeda.format(provento.valorTotal),
    },
  ]
  ...
  return (
    <div className="space-y-8">
      ...
      <DataTable
        ...
        colunas={colunasProventos}
        itens={proventos}
        obterChaveLinha={(provento) => provento.id}
        carregando={carregando}
        estadoCarregando="Carregando proventos..."
        estadoVazio="Nenhum provento importado ainda."
      />
    </div>
  )
}
